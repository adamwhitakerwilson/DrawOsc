...
/**
 * Created by adamwhitakerwilson on 2017-04-01.
 */

@SuppressWarnings("DefaultFileTemplate")
public class myCustomDraw extends AppCompatActivity {
    private float x1;
    private float y1;
    private float x2;
    private float y2;
    ...
    private DrawActivity.DrawingView dv ;
...
//                            invalidate();
    ...
    public void setConnection(){
        try {
            targetIP = InetAddress.getByName("192.168.1.66");
    ...
    //Rotates {(x1,y1), (x2,y2)...(x8,y8)} by 45 degrees

    private void adamsMath(){

        x2= (x1*((float)Math.cos(-315)))-(y1*((float)Math.sin(-315)));
        y2= (x1*((float)Math.sin(-315))+(y1*((float)Math.cos(-315))));

        //noinspection SuspiciousNameCombination
        x3= y1;
        y3= x1*(-1);

        x4= (x1*((float)Math.cos(180)))-(y1*((float)Math.sin(180)));
        y4= (x1*((float)Math.sin(180))+(y1*((float)Math.cos(180))));

        x5 = (x1*((float)Math.cos(-135)))-(y1*((float)Math.sin(-135)));
        y5 = (x1*((float)Math.sin(-135))+(y1*((float)Math.cos(-135))));
        ...
        x8= (x1*((float)Math.cos(315)))-(y1*((float)Math.sin(315)));
        y8= (x1*((float)Math.sin(315))+(y1*((float)Math.cos(315))));
